...
const Result = () => {
  ...
  //const items = txtbiblicos?.value.response
  //console.log(items)
  //<a href="/search" className="">Nova Pesquisa</a>
          ...
          <div className="flex flex-wrap flex-row items-center justify-end gap-3 
            ...
            {txtbiblicos?.value.response.length > 1 && (
              <div className='flex gap-2'>
                <p className="text-inherit text-xl font-light text-gray-900">{txtbiblicos?.value.response.length} referências encontradas.</p>
                <button
                  onClick={() => { clipb(all()) } }
                  type="button" className="flex flex-row items-center gap-1 font-medium text-inherit text-indigo-600 hover:text-indigo-700">
                  <TbCopy /> Copiar
                </button>
              </div>
            )}
                  ...
                  <div className="flex flex-1 flex-wrap items-end justify-between space-y-2">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        type="button" className="flex flex-row items-center gap-1 font-medium text-indigo-600 hover:text-indigo-500">
                        <MdShare /> Enviar
                      </button>
